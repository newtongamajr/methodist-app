<div class="space-y-6">
    <div class="flex items-center justify-between gap-4">
        <flux:heading size="xl">
            {{ $schedule ? __('Edit prayer schedule') : __('New prayer schedule') }}
        </flux:heading>
    ...
    <form wire:submit="save" class="space-y-5">
        <div class="grid gap-4 sm:grid-cols-2">
            <flux:select wire:model="church_id" :label="__('Church')" required>
            ...
            <flux:select wire:model.live="prayer_campaign_id" :label="__('Prayer campaign')" required>
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            <flux:select wire:model="slot_minutes" :label="__('Slot length')">
                <option value="30">30 {{ __('minutes') }}</option>
                <option value="60">60 {{ __('minutes') }}</option>
            </flux:select>
            <flux:input wire:model="capacity_per_slot" :label="__('Max prayers per slot')" type="number" min="1" max="200" required />
            <flux:select wire:model="mode" :label="__('Mode')">
    ...
    @if ($schedule && $schedule->slots()->count())
        <div class="space-y-3">
