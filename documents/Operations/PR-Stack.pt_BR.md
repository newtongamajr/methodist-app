...
| 22 | `assignment-roles` | `v1.02` | `main` | Novo CRUD `Admin → Structure → Assignment roles` em cima da lookup table existente `assignment_roles` (gated por `church.manage`, ao lado de Regions / Districts / Churches / Groups). Três pages Livewire MFC: um index com search + filtro de status e per-row actions (View → modal Flux com details read-only; Edit; People-with-this-role-and-which-group; Delete via o modal padrão `x-confirm-delete`), um editor com fallback de auto-slug pelo `App\Support\GenerateUniqueSlug`, e uma page People-by-role que faz join `PersonRoleAssignment` → person + group + escopo (church / district / region / national) + function com filtro de status ativo/ended. Novo `App\Livewire\Forms\AssignmentRoleForm` (valida name + slug único + description + is_active). Routes registradas sob `EnsureCanManageChurches`; menu plugado no dropdown desktop, sidebar mobile e command palette. Entrega traduções pt_BR + es de toda key nova; também preenche `Group` / `Groups` / `Edit group` / `New group` / `All groups` / `No groups yet.` / `No matching groups for this function.` / `Delete this group and end every member assignment?` / `Pick a group…` / `Group created.` / `Group updated.` e uma key `Scope` faltando que estavam stubbed em English. Cinco testes Pest novos cobrem authorization, create-com-auto-slug, update, delete, e a listagem de people-by-role |
| 23 | `paginator-translations` | `v1.03` | `main` | Traduz a linha resumo do paginador Flux. A view de paginação do Flux (`vendor/livewire/flux/stubs/resources/views/flux/pagination.blade.php`) monta a frase a partir de quatro keys top-level bare em `__()` (`Showing` / `to` / `of` / `results`) — nenhuma existia nos keysets JSON, então ficava em English sob pt_BR / es. Adiciona as quatro keys em en / pt_BR / es ("Exibindo 1 a 3 de 3 resultados" / "Mostrando 1 a 3 de 3 resultados") e traduz os labels das setas prev / next em `lang/{pt_BR,es}/pagination.php` (ainda em English vindo do stub do Laravel) |

URL do PR: https://github.com/newtongamajr/methodist-app/pull/21
URL do PR: https://github.com/newtongamajr/methodist-app/pull/22
URL do PR: https://github.com/newtongamajr/methodist-app/pull/23
