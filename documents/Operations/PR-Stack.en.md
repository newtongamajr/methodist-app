# PR stack — code review through Person Architecture Phase 3

Seven stacked PRs ship the entire trajectory from `main` up through Phase 3 of
...
**Merge order: #2 → #3 → #4 → #5 → #1 → #6 → #7.** As each PR merges, GitHub
will auto-retarget the next one in the chain to `main` (or to whatever the
new base is). Do not squash-merge — preserve the commit history so the
...
Phase 3 adds the Family tab on `/admin/people` editor and the family-tree
query helpers on Person (`parents`, `children`, `siblings`, `spouse`,
`grandparents`, `grandchildren`, `auntsAndUncles`, `niecesAndNephews`,
`cousins`, `godparents`, `godchildren`, `guardians`, `wards`, `familyTree`).
Still deferred to Phases 4–7:
...
- [ ] All seven PRs are open, in the right order, against the right base
- [ ] CI green on each (or at minimum on the topmost one — once merging starts the bases will retarget and CI re-runs)
- [ ] `php artisan migrate:fresh --seed` succeeds against the **head of the topmost PR** (#7) — proves the whole chain composes
- [ ] `php artisan test --compact` is green at HEAD of #7 (191 tests / 458 assertions at last run)
- [ ] `vendor/bin/pint --test --format agent` clean at HEAD of #7
- [ ] Translation parity: `en.json` / `pt_BR.json` / `es.json` all 507 keys at HEAD of #7
